 ...
 */
export async function callAIForAsset(
  asset: string,
  systemPrompt: string,
  userPrompt: string
  ...
  if (jsonData && jsonData.coin && jsonData.signal) {
    ...
    if (jsonData.signal) {
      ...
      if (jsonData.signals && Array.isArray(jsonData.signals) && jsonData.signals.length > 0) {
        ...
        signal = jsonData.signals.find((s: any) => s.coin === asset || !s.coin) as Signal
        if (signal && !signal.coin) {
          signal.coin = asset
        }
      } else if (jsonData.data && Array.isArray(jsonData.data) && jsonData.data.length > 0) {
        // Try data key
        signal = jsonData.data.find((s: any) => s.coin === asset || !s.coin) as Signal
        if (signal && !signal.coin) {
          signal.coin = asset
        }
      } else if (Array.isArray(jsonData) && jsonData.length > 0) {
        // Response is array, find signal for this asset or use first
        signal = jsonData.find((s: any) => s.coin === asset || !s.coin) as Signal
        if (signal && !signal.coin) {
          signal.coin = asset
        }
      }
