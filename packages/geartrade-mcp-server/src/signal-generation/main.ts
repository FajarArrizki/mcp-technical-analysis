...
export async function main(): Promise<void> {
    ...
    try {
      ...
      if (maxAssets > 0 && allowedAssets.length > maxAssets) {
        ...
        const priorityInAvailable = priorityAssets.filter((a: string) => allowedAssets.includes(a))
        const rest = allowedAssets.filter((a: string) => !priorityAssets.includes(a))
      ...
      const formattedAssets = assetsToFetch.length <= 10 
        ? assetsToFetch.map((asset: string) => `${asset}-USDC`).join(', ')
        : `${assetsToFetch.slice(0, 10).map((asset: string) => `${asset}-USDC`).join(', ')} ... (${assetsToFetch.length} total)`
      ...
      const expectedConfMap = new Map<string, { expected: number; majorMismatches: number }>()
      ...
      for (const [asset, data] of mdEntries) {
        try {
          const ec = computeExpectedConfidence(asset, data as any)
      ...
      const preRanked = rankedScores
        .map(rs => {
          ...
          if (weaknessCount > 0) preRank -= WEAKNESS_PENALTY * weaknessCount
          if (ec < EXPECTED_CONF_FLOOR) preRank -= 1e6 // hard demote low expected confidence
      ...
      log(`   Assets: ${allowedAssets.map((asset: string) => `${asset}-USDC`).join(', ')}`, 'cyan')
      ...
      if (signalWarnings.length > 0) {
        ...
        const warningsByAsset: Record<string, any[]> = {}
        signalWarnings.forEach((warning: { asset: string; message: string }) => {
          ...
          assetWarnings.forEach((warning, idx) => {
            ...
            if (warning.details) {
              if (Array.isArray(warning.details)) {
                warning.details.forEach((detail: string | null | undefined) => {
      ...
      const qualifyingSignals = signalsWithReward.filter(signal => {
        ...
        const hasCriticalVolumeContradiction = contradictions.some(contra => 
        ...
        const hasVolumeContradiction = contradictions.some(contra => 
          (contra.includes('volume spike') || contra.includes('volume drop')) &&
          !contra.includes('CRITICAL:')
        )
        ...
        const baseThreshold = Math.max(
          ...
          isFinite(MIN_CONF_OVERALL) ? MIN_CONF_OVERALL : HIGH_CONFIDENCE_THRESHOLD,
          tierMin
        )
      ...
      function computeCombinedScore(s: any): { rank: number, info: string } {
        ...
        if (rewardRes && rewardRes.reward) reward += rewardRes.reward
        ...
        if (gap > GAP_MAX) penalty += (gap - GAP_MAX)
        if (ecGap > EC_GAP_MAX) penalty += (ecGap - EC_GAP_MAX)
        // Consistency bonuses
        const GAP_BONUS_MAX = parseFloat(process.env.GAP_BONUS_MAX || '10')
        const EC_BONUS_MAX = parseFloat(process.env.EC_BONUS_MAX || '10')
        if (gap <= GAP_MAX) reward += Math.min(GAP_BONUS_MAX, Math.max(0, GAP_MAX - gap))
        if (ecGap <= EC_GAP_MAX) reward += Math.min(EC_BONUS_MAX, Math.max(0, EC_GAP_MAX - ecGap))
        // Autoban on major mismatches
        if (conflict.majorMismatches >= MAJOR_MISMATCH_AUTOBAN) penalty += 1e6
