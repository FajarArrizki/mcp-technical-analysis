...
export function generateInvalidationCondition(
  ...
  entryPrice: number,
  stopLoss: number,
  supportResistance: { support?: number; resistance?: number; supportLevels?: number[]; resistanceLevels?: number[] } | null | undefined,
  ...
  const supportLevels = supportResistance?.supportLevels || (indicators as any).supportLevels || []
  const resistanceLevels = supportResistance?.resistanceLevels || (indicators as any).resistanceLevels || []
  ...
  if (signalType === 'buy_to_enter' || signalType === 'add') {
    ...
    if (supportLevels.length > 0) {
      supportLevels.forEach((support: number) => {
    ...
    if (indicators.candlestickPatterns && 
      ...
      if (bullishPatterns.length > 0) {
        conditions.push(`Bullish candlestick pattern fails to confirm (${bullishPatterns.map((p: any) => p.type).join(', ')})`)
      }
    }
    ...
    if (resistanceLevels.length > 0) {
      resistanceLevels.forEach((resistance: number) => {
    ...
    if (indicators.candlestickPatterns && 
      ...
      if (bearishPatterns.length > 0) {
        conditions.push(`Bearish candlestick pattern fails to confirm (${bearishPatterns.map((p: any) => p.type).join(', ')})`)
      }
    }
