...
import { PositionSizingStrategy, PositionSizeResult, Signal } from '../types'

export interface PositionSizerConfig {
  strategy: PositionSizingStrategy
  totalCapital: number
  maxPositionSizePct: number // Maximum position size per asset (default: 20%)
  reserveCapitalPct: number // Reserve capital for new signals (default: 10%)
  topNRanking?: number // Current ranking of asset (for ranking_weighted strategy)
  assetCorrelation?: Record<string, number> // Correlation matrix (for risk_parity strategy)
  winRate?: number // Win rate for Kelly criterion (0-1)
  averageWin?: number // Average win percentage for Kelly criterion
  averageLoss?: number // Average loss percentage for Kelly criterion
}
...
export function calculatePositionSize(
  ...
  existingPositionsCount: number = 0
): PositionSizeResult {
