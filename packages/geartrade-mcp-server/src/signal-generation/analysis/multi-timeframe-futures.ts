...
export interface MultiTimeframeFuturesResult {
  timeframes: Map<string, MultiTimeframeFuturesAnalysis>
  alignmentScore: number // 0-100, how well timeframes align
  consensusSignal: 'long' | 'short' | 'neutral'
 ...
 */
function determineConsensusSignal(
  timeframes: Map<string, MultiTimeframeFuturesAnalysis>
): 'long' | 'short' | 'neutral' {
  const signals = Array.from(timeframes.values()).map(tf => tf.signal)
 ...
 */
export function analyzeMultiTimeframeFutures(
  futuresDataMap: Map<string, FuturesMarketData> | Record<string, FuturesMarketData>,
  priceChanges?: Map<string, number> | Record<string, number>,
  volumeChanges?: Map<string, number> | Record<string, number>
): MultiTimeframeFuturesResult {
  const timeframes = new Map<string, MultiTimeframeFuturesAnalysis>()

  // Analyze each timeframe
  const timeframeKeys: Array<'5m' | '15m' | '1h' | '4h' | '1d'> = ['5m', '15m', '1h', '4h', '1d']
