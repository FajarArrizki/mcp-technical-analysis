...
import type { TechnicalIndicators } from '../technical-indicators/aggregator'

export interface IndicatorCountResult {
  bullishCount: number
  bearishCount: number
  summaryDir: 'BUY' | 'SELL' | 'MIXED'
}
 ...
 */
export function countBullishBearishIndicators(
  indicators: TechnicalIndicators,
  currentPrice: number
): IndicatorCountResult {
  let bullishCount = 0
  let bearishCount = 0
  ...
  if (indicators.volumeChange !== null && indicators.volumeChange !== undefined) {
    if (indicators.volumeChange > 10) bullishCount++ // Volume increased significantly = bullish
    else if (indicators.volumeChange < -10) bearishCount++ // Volume decreased significantly = bearish
    // Volume change -10% to +10%: neutral, don't count
  }

  // Bollinger Bands Middle
  if (indicators.bollingerBands && price > 0) {
    if (price > indicators.bollingerBands.middle) bullishCount++
    else bearishCount++
  }

  // Parabolic SAR
  if (indicators.parabolicSAR !== null && indicators.parabolicSAR !== undefined && price > 0) {
    if (price > indicators.parabolicSAR) bullishCount++
    else bearishCount++
  }

  // Aroon
  if (indicators.aroon) {
    if (indicators.aroon.up > indicators.aroon.down) bullishCount++
    else bearishCount++
  }

  // CCI
  if (indicators.cci !== null && indicators.cci !== undefined) {
    if (indicators.cci > 100) bullishCount++
    else if (indicators.cci < -100) bearishCount++
  }

  // VWAP
  if (indicators.vwap !== null && indicators.vwap !== undefined && price > 0) {
    if (price > indicators.vwap) bullishCount++
    else bearishCount++
  }

  // 24h Price Change
  if (indicators.priceChange24h !== null && indicators.priceChange24h !== undefined) {
    if (indicators.priceChange24h > 0) bullishCount++
    else bearishCount++
  }
  ...
  if (indicators.rsi14 !== null && indicators.rsi14 !== undefined) {
    if (indicators.rsi14 < 30) bullishCount++ // Oversold = bullish reversal potential
    else if (indicators.rsi14 > 70) bearishCount++ // Overbought = bearish reversal potential
  ...
  if (indicators.stochastic?.k !== null && indicators.stochastic?.k !== undefined) {
    ...
    if (stochK < 20) bullishCount++ // Oversold = bullish reversal potential
    else if (stochK > 80) bearishCount++ // Overbought = bearish reversal potential
  ...
  if (indicators.williamsR !== null && indicators.williamsR !== undefined) {
    if (indicators.williamsR < -80) bullishCount++ // Oversold = bullish reversal potential
    else if (indicators.williamsR > -20) bearishCount++ // Overbought = bearish reversal potential
  ...
  if (indicators.rsiDivergence?.divergence) {
    ...
    if (divLower.includes('bullish')) bullishCount++
    else if (divLower.includes('bearish')) bearishCount++
  }

  // Calculate summary direction
  const summaryDir: 'BUY' | 'SELL' | 'MIXED' = 
