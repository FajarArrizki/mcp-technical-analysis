...
export interface WeightedIndicatorScore {
  name: string
  weight: number
  category: string
...
export interface QualityWeightedResult {
  ...
  bullishCount: number
  bearishCount: number
  uniqueBullishCount: number // After removing redundancy
  uniqueBearishCount: number
  bullishIndicators: WeightedIndicatorScore[]
  bearishIndicators: WeightedIndicatorScore[]
  contradictions: string[]
  redundantGroups: string[]
  qualityRatio: number // bullishScore / (bullishScore + bearishScore)
...
export function calculateQualityWeightedJustification(
  ...
  const bearishIndicators: WeightedIndicatorScore[] = []
  let bullishScore = 0
  let bearishScore = 0
  ...
  const signalType = signal.signal === 'reduce' || signal.signal === 'close_all' || signal.signal === 'hold' 
    ...
    : signal.signal
  const contradictions = detectContradictions(indicators, signalType as 'buy_to_enter' | 'sell_to_enter' | 'add')
   ...
   */
  const applyContextAwareWeighting = (baseWeight: number, category: string): number => {
    if (marketRegime === 'trending') {
      // ADX more important in trending market
      if (category === 'Trend Momentum') return baseWeight * 1.3
      // Overbought less relevant in trending
      if (category === 'Overbought/Oversold') return baseWeight * 0.7
    }
    
    if (marketRegime === 'ranging' || marketRegime === 'choppy') {
      // Oscillators more reliable in ranging market
      if (category === 'Overbought/Oversold') return baseWeight * 1.3
      // Trend strength less important
      if (category === 'Trend Momentum') return baseWeight * 0.7
    }
    
    if (volatility === 'high') {
      // Divergence can be false signal in high volatility
      if (category === 'Divergence') return baseWeight * 0.8
    }
  ...
  if (indicators.macd?.histogram !== undefined) {
    ...
    if (hist > 0 && isBuySignal) {
      const score = { name: 'MACD Histogram', weight, category: 'Trend Momentum', 
                     impact: 'low' as const, description: `MACD histogram +${hist.toFixed(4)} (bullish momentum)` }
      bullishIndicators.push(score)
      bullishScore += weight
    } else if (hist < 0 && !isBuySignal) {
      const score = { name: 'MACD Histogram', weight, category: 'Trend Momentum', 
                     impact: 'low' as const, description: `MACD histogram ${hist.toFixed(4)} (bearish momentum)` }
      ...
      const score = { name: 'MACD Histogram', weight, category: 'Trend Momentum', 
                     impact: 'low' as const, description: `MACD histogram +${hist.toFixed(4)} (CONTRADICTS SELL - bullish momentum)` }
      bullishIndicators.push(score)
      ...
      const score = { name: 'MACD Histogram', weight, category: 'Trend Momentum', 
                     impact: 'low' as const, description: `MACD histogram ${hist.toFixed(4)} (CONTRADICTS BUY - bearish momentum)` }
      bearishIndicators.push(score)
  ...
  const pricePositionIndicators: WeightedIndicatorScore[] = []
   ...
   */
  const calculateAdjustedConfidence = (base: number, severity: ConflictSeverity, redundancy: number): number => {
    let adjusted = base
