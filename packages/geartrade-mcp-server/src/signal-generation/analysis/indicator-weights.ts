 ...
 */
export const INDICATOR_WEIGHTS: Record<string, WeightedIndicator> = {
 ...
 */
export function getIndicatorWeight(indicatorKey: string): number {
 ...
 */
export function areIndicatorsRedundant(indicator1: string, indicator2: string): boolean {
...
export enum ConflictSeverity {
  CRITICAL = 3.0,  // 3+ major bearish signals
  HIGH = 2.0,      // 2 major bearish signals
  MEDIUM = 1.5,    // 1 major contradiction
  LOW = 1.0        // Minor inconsistencies
}
...
export interface Contradiction {
  ...
  severityScore: number  // Added for severity calculation
  description: string
  indicators: string[]
}
...
export function detectContradictions(
  ...
  signal: 'buy_to_enter' | 'sell_to_enter' | 'add' | 'reduce' | 'close'
): Contradiction[] {
  const contradictions: Contradiction[] = []
  
  if (!indicators) return contradictions
 ...
 */
export function calculateConflictSeverity(contradictions: Contradiction[]): ConflictSeverity {
