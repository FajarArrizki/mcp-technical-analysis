...
import { BTCCorrelationData, BTCDominanceData } from '../types/futures-types'
...
const correlationCache = new Map<string, { data: BTCCorrelationData; timestamp: number }>()
 ...
 */
export async function calculateBTCCorrelation(
  asset: string,
  assetHistoricalData: HistoricalDataPoint[],
  btcHistoricalData?: HistoricalDataPoint[]
): Promise<BTCCorrelationData> {
  // Check cache first
  ...
  try {
    ...
    if (!btcData || btcData.length === 0 || !assetHistoricalData || assetHistoricalData.length === 0) {
      ...
      const neutralData: BTCCorrelationData = {
        correlation24h: 0,
        correlation7d: 0,
        correlation30d: 0,
        strength: 'weak',
    ...
    const alignedData: Array<{ assetPrice: number; btcPrice: number; time: number }> = []
    ...
    const result: BTCCorrelationData = {
 ...
 */
export function predictAltImpactFromBTCMove(
  btcMovePct: number,
 ...
 */
export function isBTCMoveSignificant(
  btcMovePct: number,
  correlationData: BTCCorrelationData,
  threshold: number = 2 // 2% BTC move threshold
): boolean {
 ...
 */
export async function batchCalculateBTCCorrelations(
  assets: string[],
  assetHistoricalDataMap: Map<string, HistoricalDataPoint[]> | Record<string, HistoricalDataPoint[]>
): Promise<Map<string, BTCCorrelationData>> {
  const results = new Map<string, BTCCorrelationData>()

  // Fetch BTC data once for all assets
  let btcHistoricalData: HistoricalDataPoint[] | undefined
  try {
 ...
 */
export const BTC_CORRELATION_EXPLANATION: string =
  [
