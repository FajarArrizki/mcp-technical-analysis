...
export interface FuturesConfidenceContext {
  signal: 'LONG' | 'SHORT'
  currentPrice: number
 ...
 */
function scoreFundingRate(
  futuresData: FuturesMarketData,
  context: FuturesConfidenceContext
): { score: number; reasons: string[] } {
  const fundingData = futuresData.fundingRate
  const indicators = calculateFundingRateIndicators(
    fundingData,
    futuresData.openInterest.change24h,
    context.priceChange24h || 0
  )
  ...
  if (indicators.extreme.isExtreme) {
    if (
      (indicators.extreme.level === 'extreme_high' && context.signal === 'SHORT') ||
  ...
  if (indicators.meanReversion.signal) {
    if (
      (indicators.meanReversion.direction === 'long' && context.signal === 'LONG') ||
...
function scoreOpenInterest(
  ...
  const indicators = calculateOpenInterestIndicators(
    oiData,
    context.priceChange24h || 0,
    context.volumeChange24h || 0
  )
  ...
  if (indicators.divergence.vsPrice.detected) {
    if (
      (indicators.divergence.vsPrice.type === 'bullish' && context.signal === 'LONG') ||
 ...
 */
function scoreLiquidationZones(
  futuresData: FuturesMarketData,
  context: FuturesConfidenceContext
): { score: number; reasons: string[] } {
  ...
  if (indicators.safeEntry.zones.length > 0) {
    const inSafeZone = indicators.safeEntry.zones.some(zone => 
      context.currentPrice >= zone.priceLow && context.currentPrice <= zone.priceHigh
    )
    if (inSafeZone) {
      score += 8
...
function scoreLongShortRatio(
  ...
  if (indicators.contrarian.signal) {
    if (
      (indicators.contrarian.direction === 'long' && context.signal === 'LONG') ||
  ...
  if (indicators.extreme.detected) {
    if (
      (indicators.extreme.level === 'extreme_short' && context.signal === 'LONG') ||
  ...
  if (indicators.divergence.signal === 'fade_retail') {
    ...
    if (
      (ratioData.retailLongPct > 60 && context.signal === 'SHORT') ||
...
export function calculateFuturesConfidence(
  ...
  const signalContext: FuturesConfidenceContext = context || {
    signal: 'LONG', // Default
