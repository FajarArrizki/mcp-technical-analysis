...
export interface IndicatorQualityScore {
  asset: string
  score: number
  ...
  // Composite ranking components (for logging/analysis)
  coveragePct?: number
  momentumComposite?: number
  trendAlignmentScore?: number
  externalDataScore?: number
  compositeScore?: number
  aggressiveScoreRaw?: number
  aggressivePercent?: number
  // AggressiveV2 diagnostics/inputs
  m60Pct?: number
  absM60Pct?: number
  volumeRatio?: number
  srDistPct?: number
  btc60mAbsPct?: number
  fundingAbsPct?: number
 ...
 */
export function scoreIndicatorQuality(
  asset: string,
  ...
  const futures = (marketData as any)?.externalData?.futures || null
  const btcCorr = futures?.btcCorrelation || null
  const cva = (marketData as any)?.externalData?.comprehensiveVolumeAnalysis || null
  ...
  if (futures?.premiumIndex && typeof (futures.premiumIndex as any).divergence === 'number') {
    indicatorsCount++
    const div = Math.abs((futures.premiumIndex as any).divergence)
    if (div < 0.5) {
      score += 3
  ...
  if (futures?.liquidation && typeof futures.liquidation.liquidationDistance === 'number') {
    ...
    if (d >= 5) score += 2
    else if (d < 2) weaknesses.push('Near liquidation cluster (<2%)')
  }
  // 13d. Delta/CVD confirmation (0-5)
  if ((cva as any)?.volumeConfirmation) {
    indicatorsCount++
    const vc = (cva as any).volumeConfirmation
    if (vc.isValid) {
  ...
  try {
    ...
    if (rb && rb.reward > 0) {
      indicatorsCount++
      score += rb.reward
      if (Array.isArray(rb.reasons) && rb.reasons.length > 0) strengths.push(...rb.reasons.slice(0, 2))
    }
  } catch (_) {
    // no-op
  }
  ...
  function qualityFromCoverageAndScore(
    coverage: number,
    scoreValue: number
  ): 'excellent' | 'very good' | 'good' | 'fair' | 'poor' | 'very poor' {
    // <50% → Very Poor
    if (coverage < 50) return 'very poor'
    // 50-59% → Poor (any score)
    if (coverage < 60) return 'poor'
    // 60-69% → 120+ Fair else Poor
    if (coverage < 70) return scoreValue >= 120 ? 'fair' : 'poor'
    // 70-79% → 130+ Good, 110-129 Fair, else Poor
    if (coverage < 80) {
      if (scoreValue >= 130) return 'good'
      if (scoreValue >= 110) return 'fair'
      return 'poor'
    }
    // 80-89% → 140+ Very Good, 120-139 Good, else Fair
    if (coverage < 90) {
      if (scoreValue >= 140) return 'very good'
      if (scoreValue >= 120) return 'good'
      return 'fair'
    }
    // 90-100% → 150+ Excellent, 140-149 Very Good, 130-139 Good, 120-129 Fair, <120 Poor
    if (scoreValue >= 150) return 'excellent'
    if (scoreValue >= 140) return 'very good'
    if (scoreValue >= 130) return 'good'
 ...
 */
export function rankAssetsByIndicatorQuality(
  marketData: Map<string, MarketData> | Record<string, MarketData>,
  allowedAssets?: string[] // OPTIMIZATION: Only rank specific assets if provided
): IndicatorQualityScore[] {
  const scores: IndicatorQualityScore[] = []
  ...
  for (const [asset, data] of filteredArray) {
    ...
    if ((base.indicatorsCount || 0) === 0) {
      ...
      if (skippedNoIndicators <= 5) {
        const hasHist = !!(data as any)?.historicalData && Array.isArray((data as any).historicalData) && (data as any).historicalData.length > 0
        const hasInd = !!(data as any)?.indicators && typeof (data as any).indicators === 'object'
        const hasPrice = !!(data as any)?.price || !!(data as any)?.markPx
    ...
    const historical = (data as any)?.historicalData || []
    const hl = (data as any)?.externalData?.hyperliquid || {}
    ...
    const ta = (data as any)?.trendAlignment || {}
    ...
    const ext = (data as any)?.externalData || {}
    ...
    const dailyTrend = (data as any)?.trendAlignment?.dailyTrend
    ...
    const h4Aligned = (data as any)?.trendAlignment?.h4Aligned
    const h1Aligned = (data as any)?.trendAlignment?.h1Aligned
    if ((data as any)?.trendAlignment && (data as any).trendAlignment.aligned === false) {
    ...
    const indicatorsAny = (data as any)?.indicators || {}
    ...
    const cva = (ext as any)?.comprehensiveVolumeAnalysis
    ...
    if (dailyTrend === 'uptrend' && m60c > 0) composite = Math.min(1, composite + rewardMomoTrend)
    if (dailyTrend === 'downtrend' && m60c < 0) composite = Math.min(1, composite + rewardMomoTrend)
    if (aligned) composite = Math.min(1, composite + rewardAlign)
    if (typeof buyPressure === 'number' && typeof sellPressure === 'number') {
      if (buyPressure > sellPressure && momoPct > 0) composite = Math.min(1, composite + rewardVolConfirm)
      if (sellPressure > buyPressure && momoPct < 0) composite = Math.min(1, composite + rewardVolConfirm)
    }
    ...
    const hist = (data as any)?.historicalData || []
    if (typeof ind?.volume === 'number' && Array.isArray(hist) && hist.length > 1) {
      ...
      for (let i = hist.length - 1 - useN; i < hist.length - 1; i++) {
        ...
        if (typeof v === 'number' && isFinite(v) && v > 0) { sum += v; cnt++ }
      }
      const avg = cnt > 0 ? sum / cnt : 0
      if (avg > 0) volumeRatio = ind.volume / avg
    }
    // S/R distance using supportLevels/resistanceLevels if present
    let srDistPct: number | undefined
    const priceNow = typeof ind?.price === 'number' ? ind.price : undefined
    if (typeof priceNow === 'number' && isFinite(priceNow) && priceNow > 0) {
      const supLevels: number[] = Array.isArray(ind?.supportLevels) ? ind.supportLevels : []
      const resLevels: number[] = Array.isArray(ind?.resistanceLevels) ? ind.resistanceLevels : []
      const dists: number[] = []
      for (const s of supLevels) if (typeof s === 'number' && isFinite(s) && s > 0) dists.push(Math.abs((priceNow - s) / priceNow) * 100)
      for (const r of resLevels) if (typeof r === 'number' && isFinite(r) && r > 0) dists.push(Math.abs((r - priceNow) / priceNow) * 100)
      if (dists.length > 0) srDistPct = Math.min(...dists)
    }
    // Funding abs (%)
    let fundingAbsPct = 0
    const fr = (data as any)?.externalData?.futures?.fundingRate
    if (fr && typeof fr.current === 'number') fundingAbsPct = Math.abs(fr.current) * 100
    // Confidence already computed above as confidencePercent
    // BTC 60m abs pct computed from BTC marketData if available
    let btc60mAbsPct = 0
    try {
      const btcData = marketData instanceof Map ? (marketData as Map<string, any>).get('BTC') : (marketData as any)['BTC']
      ...
      if (typeof btc60 === 'number' && isFinite(btc60)) btc60mAbsPct = Math.abs(btc60)
    } catch (_) {
      // leave 0
    }
 ...
 */
export function selectTopAssetsByIndicatorQuality(
  marketData: Map<string, MarketData> | Record<string, MarketData>,
  topN: number = 15,
  allowedAssets?: string[] // OPTIMIZATION: Only rank specific assets if provided
): string[] {
...
function pctChangeOverMinutesFromHistorical(historical: any[], minutes: number): number | null {
  try {
