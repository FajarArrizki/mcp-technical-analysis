...
export interface AnchoredVWAPData {
  ...
  // Current price vs Anchored VWAP
  priceVsVWAP: number  // % difference
  position: 'above' | 'below' | 'equal'

  // Trend based on position vs VWAP
  trend: 'bullish' | 'bearish' | 'neutral'

  // Standard deviation bands
  upperBand: number    // VWAP + 1 SD
  lowerBand: number    // VWAP - 1 SD

  // Position relative to bands
  bandPosition: 'above_upper' | 'between_bands' | 'below_lower' | 'at_vwap'
...
export function calculateAnchoredVWAP(
  ...
  let bandPosition: 'above_upper' | 'between_bands' | 'below_lower' | 'at_vwap'
 ...
 */
export function findAnchorPoints(
  highs: number[],
  lows: number[],
  closes: number[]
): number[] {
 ...
 */
export function calculateMultipleAnchoredVWAP(
  highs: number[],
  lows: number[],
  closes: number[],
  volumes: number[],
  ...
  return anchorIndices
    ...
    .filter((vwap): vwap is AnchoredVWAPData => vwap !== null)
}
 ...
 */
export function getAnchoredVWAPInterpretation(vwap: AnchoredVWAPData): string {
 ...
 */
export function analyzeVWAPTrendStrength(vwap: AnchoredVWAPData): {
