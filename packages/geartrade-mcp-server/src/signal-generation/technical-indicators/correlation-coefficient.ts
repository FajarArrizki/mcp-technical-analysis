...
export interface CorrelationCoefficientData {
  ...
  // Trend relationship
  trendRelationship: 'converging' | 'diverging' | 'neutral'

  // Signal strength (0-100)
  signalStrength: number

  // Trading signal based on correlation
  signal: 'buy' | 'sell' | 'neutral'

  // Risk assessment
  diversificationBenefit: 'high' | 'moderate' | 'low'
}
...
export function calculateCorrelationCoefficient(
  ...
  prices2: number[],
  period: number = 30
): CorrelationCoefficientData | null {
  ...
  let trendRelationship: 'converging' | 'diverging' | 'neutral' = 'neutral'

  if (prices1.length >= period * 2 && prices2.length >= period * 2) {
    const recentCorr = calculateCorrelationCoefficient(
      prices1.slice(-period),
      prices2.slice(-period),
      period
    )
    const olderCorr = calculateCorrelationCoefficient(
      prices1.slice(-period * 2, -period),
      prices2.slice(-period * 2, -period),
      period
    )
  ...
  let diversificationBenefit: 'high' | 'moderate' | 'low' = 'low'
 ...
 */
export function calculateMultipleCorrelations(
  prices1: number[],
  prices2: number[],
  periods: number[] = [20, 30, 60, 90]
): CorrelationCoefficientData[] {
  return periods
    .map(period => calculateCorrelationCoefficient(prices1, prices2, period))
    .filter((corr): corr is CorrelationCoefficientData => corr !== null)
}
 ...
 */
export function getCorrelationInterpretation(corr: CorrelationCoefficientData): string {
 ...
 */
export function analyzeCorrelationStability(
  prices1: number[],
  prices2: number[],
  analysisPeriod: number = 60
): {
  stability: 'very_stable' | 'stable' | 'unstable' | 'very_unstable'
  ...
  const correlationVolatility = Math.sqrt(
    correlations.reduce((sum, corr) => sum + Math.pow(corr - averageCorrelation, 2), 0) / correlations.length
  )

  // Determine stability
  let stability: 'very_stable' | 'stable' | 'unstable' | 'very_unstable'
