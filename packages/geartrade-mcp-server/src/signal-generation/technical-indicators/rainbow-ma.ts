...
export interface RainbowMAData {
  // Individual MA values (typically 8-10 MAs)
  ma2: number    // Fastest MA (violet)
  ma3: number    // Red
  ma4: number    // Orange
  ma5: number    // Yellow
  ma6: number    // Green
  ma7: number    // Blue
  ma8: number    // Indigo
  ma9: number    // Violet (slowest)

  // Trend analysis
  trend: 'strong_bullish' | 'bullish' | 'neutral' | 'bearish' | 'strong_bearish'

  // Rainbow alignment (all MAs in order)
  alignment: 'bullish_alignment' | 'bearish_alignment' | 'mixed'

  // Current price position vs rainbow
  position: 'above_rainbow' | 'in_rainbow' | 'below_rainbow'

  // Trend strength (0-100)
  strength: number

  // Fast vs slow MA spread
  spread: number          // Difference between fastest and slowest MA (%)
  compression: boolean    // Whether the rainbow is compressing (potential breakout)
}
 ...
 */
export function calculateRainbowMA(
  prices: number[],
  periods: number[] = [2, 3, 4, 5, 6, 7, 8, 9]
): RainbowMAData | null {
  ...
  let alignment: 'bullish_alignment' | 'bearish_alignment' | 'mixed'
  ...
  let position: 'above_rainbow' | 'in_rainbow' | 'below_rainbow'
  ...
  if (alignment === 'bullish_alignment') {
    ...
    if (position === 'above_rainbow') strength += 20
  } else if (alignment === 'bearish_alignment') {
    strength -= bearishCount * 5
    if (position === 'below_rainbow') strength -= 20
  }
 ...
 */
export function getRainbowMASignal(rainbow: RainbowMAData): 'buy' | 'sell' | 'neutral' {
 ...
 */
export function getRainbowMASlope(rainbow: RainbowMAData): number {
 ...
 */
export function isRainbowMAFan(rainbow: RainbowMAData): boolean {
  const mas = [rainbow.ma2, rainbow.ma3, rainbow.ma4, rainbow.ma5,
               rainbow.ma6, rainbow.ma7, rainbow.ma8, rainbow.ma9]
