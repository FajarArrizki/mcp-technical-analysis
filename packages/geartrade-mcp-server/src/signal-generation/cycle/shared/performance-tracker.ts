...
export interface PerformanceTrackerConfig {
  ...
  equityCurveUpdateIntervalMs?: number
}

export class PerformanceTracker {
  private config: PerformanceTrackerConfig
  private trades: TradeRecord[]
  private equityCurve: Array<{ timestamp: number; equity: number; peak: number; drawdownPct: number }>
  private startEquity: number
  private currentEquity: number
  private peakEquity: number

  constructor(config: PerformanceTrackerConfig, initialEquity: number = 10000) {
    this.config = config
    this.trades = []
    this.equityCurve = []
   ...
   */
  private updateEquityCurve(trade: TradeRecord): void {
  ...
  getMetrics(): PerformanceMetrics {
    ...
    // Trades per asset
    const tradesPerAsset: Record<string, {
      totalTrades: number
      winRate: number
