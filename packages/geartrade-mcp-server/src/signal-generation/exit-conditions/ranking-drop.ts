...
export interface RankingDropConfig {
  enabled: boolean
  topN: number // Current top N assets
  confirmationCycles: number // Number of cycles asset must be out of Top N before exit (default: 2)
  useBuffer: boolean // Use buffer zone (Top N+2) instead of strict Top N
  bufferSize: number // Buffer size if buffer enabled (default: 2)
}

export function checkRankingDrop(
  position: PositionState,
  currentRanking: number | null, // Current rank of the asset (null if not in ranking)
  config: RankingDropConfig
): ExitCondition | null {
  ...
  return {
    reason: 'RANKING_DROP' as ExitReason,
...
export function updateRankingHistory(
  position: PositionState,
