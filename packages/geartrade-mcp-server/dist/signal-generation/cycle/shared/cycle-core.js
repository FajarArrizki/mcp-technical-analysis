 ...
 */
export async function executeCycle(currentState, config) {
    const cycleId = `cycle_${Date.now()}`;
    const now = Date.now();
    try {
        // Step 1: Check circuit breaker
        const circuitBreakerCheck = checkCircuitBreaker(currentState.circuitBreaker || { status: 'NORMAL', dailyPnL: 0, consecutiveLosses: 0, apiErrorRate: 0 }, {
            dailyLossLimitPct: 5,
            consecutiveLossesLimit: 5,
            apiErrorRateThreshold: 50
        }, currentState.trades || [], config.executor.getApiErrorRate ? config.executor.getApiErrorRate() : 0, config.executor.getApiRequestCount ? config.executor.getApiRequestCount() : 0);
        ...
        if (config.isLive && currentState.positions) {
            try {
                const syncResult = await syncPositionsWithHyperliquid(currentState.positions, getHyperliquidAccountAddress(), { enabled: true, importManualOpens: false });
        ...
        const rankedScores = rankAssetsByIndicatorQuality(normalizedMarketData, allowedAssets);
        ...
        console.log(`\x1b[90m${'─'.repeat(296)}\x1b[0m`);
        // Helpers
        const pctChangeOverMinutes = (assetData, minutes) => {
            try {
        ...
        let computeExpectedConfidenceFn = null;
        try {
        ...
        for (let i = 0; i < displayCount; i++) {
            ...
            const assetDataObj = marketData instanceof Map ? marketData.get(ranked.asset) : marketData[ranked.asset];
            ...
            const rawPriceString = assetDataObj?.data?.priceString ||
                assetDataObj?.data?.markPxString ||
                assetDataObj?.priceString ||
                assetDataObj?.markPxString ||
            ...
            const alignPctDisplay = `${Math.max(0, Math.min(100, ranked.trendAlignmentScore || 0)).toFixed(0)}%`.padEnd(10);
            const compPctDisplay = `${Math.max(0, Math.min(100, Math.round((ranked.compositeScore || 0) * 100)))}%`.padEnd(8);
            const aggrPctDisplay = `${Math.max(0, Math.min(100, Math.round((ranked.aggressivePercent || 0))))}%`.padEnd(8);
            ...
            if (typeof currentPx === 'number' && isFinite(currentPx) && currentPx > 0) {
                const dists = [];
                if (typeof support === 'number' && isFinite(support) && support > 0)
                    dists.push(Math.abs((currentPx - support) / currentPx) * 100);
                if (typeof resistance === 'number' && isFinite(resistance) && resistance > 0)
                    dists.push(Math.abs((resistance - currentPx) / currentPx) * 100);
                if (dists.length > 0)
            ...
            const coloredFunding = (() => {
                ...
                if (!isNaN(v)) {
                    if (v < 0.05)
                        return green(padded); // near-neutral funding is good
                    if (v > 0.15)
                        return red(padded); // elevated funding = risk
                }
                return base(padded);
            })();
            const coloredSRDist = (() => {
                ...
                if (v != null) {
                    if (v < 0.3)
                        return red(padded); // too close = risky
                    if (v >= 0.3 && v <= 2.5)
                        return green(padded); // actionable zone
                }
                return base(padded);
            })();
            const coloredM60 = (() => {
            ...
            const btcAssetObj = marketData instanceof Map ? marketData.get('BTC') : marketData['BTC'];
            ...
            const coloredBTC = (() => {
                if (typeof btcPct60 === 'number' && isFinite(btcPct60)) {
                    if (btcPct60 > 0)
                        return `\x1b[32m${btcPadded}\x1b[0m`;
                    if (btcPct60 < 0)
                        return `\x1b[31m${btcPadded}\x1b[0m`;
                }
                return btcPadded;
            })();
            ...
            if (typeof m60 === 'number' && isFinite(m60)) {
                if (m60 > 0.5)
                    countDiff += 1; // Strong bullish momentum
                else if (m60 < -0.5)
                    countDiff -= 1; // Strong bearish momentum
            }
            if (typeof m15 === 'number' && isFinite(m15)) {
                if (m15 > 0)
                    countDiff += 0.5; // Bullish momentum
                else if (m15 < 0)
                    countDiff -= 0.5; // Bearish momentum
            }
        ...
        let positions = currentState.positions || new Map();
        if (positions.size > 0) {
            positions = updatePositionPrices(positions, normalizedMarketData);
        }
        ...
        const signals = await generateSignals(config.model, marketData, accountState, assetsToUse, // Use filtered assets (or fallback to topAssets if all filtered)
        ...
        if (positions.size > 0) {
            for (const [symbol, position] of positions.entries()) {
                ...
                const exitConditions = checkAllExitConditions(position, position.currentPrice, matchingSignal || null, // Pass signal for signal reversal check
                currentRanking, assetMarketData || null, {
        ...
        for (const { position, exitAction } of exitActions) {
            ...
            try {
                ...
                const order = await config.executor.executeExit(position, exitAction.exitSize, exitAction.reason, exitAction.exitPrice);
                if (order.status === 'FILLED' || order.status === 'PARTIAL_FILLED') {
                    ...
                    console.warn(`⚠️  Exit order ${order.status}: ${symbol} - ${order.rejectedReason || 'Unknown reason'}`);
                }
            }
            catch (error) {
                const errorMsg = error instanceof Error ? error.message : String(error);
            ...
            for (const signal of signals) {
                ...
                try {
                    ...
                    const srDistPct = (() => {
                        ...
                        const dists = [];
                        if (sup != null)
                            dists.push(Math.abs((px - sup) / px) * 100);
                        if (res != null)
                            dists.push(Math.abs((res - px) / px) * 100);
                    ...
                    if (signal.signal === 'buy_to_enter') {
                        ...
                        const isPumpPattern = typeof m60 === 'number' && isFinite(m60) && m60 >= pumpM60Threshold &&
                        ...
                        const nearResistance = typeof resistance === 'number' && isFinite(resistance) && resistance > 0 &&
                        ...
                        const momentumExhaustion = typeof m5 === 'number' && m5 > 0 &&
                    ...
                    if (signal.signal === 'sell_to_enter') {
                        ...
                        const isBleedPattern = typeof m60 === 'number' && isFinite(m60) && m60 <= bleedM60Threshold &&
                            typeof m5 === 'number' && isFinite(m5) && m5 < 0 &&
                            typeof m15 === 'number' && isFinite(m15) && m15 < 0 &&
                ...
                if (!config.executor.executeEntry)
                    continue;
                try {
                    const assetMarketData = marketData instanceof Map ? marketData.get(signal.coin || '') : marketData[signal.coin || ''];
        ...
        const rejectedSignals = global.__rejectedSignals || [];
        // Clear global after reading to avoid memory leak
        if (global.__rejectedSignals) {
            delete global.__rejectedSignals;
        }
        ...
        for (const s of signals || []) {
            if (s && typeof s.confidence === 'number') {
                let c = s.confidence;
                if (c > 1)
                    c = c / 100; // handle percentage inputs like 72.3 → 0.723
                if (c < 0)
                    c = 0;
                if (c > 1)
                    c = 1;
        ...
        const filteredSignalsForReturn = [];
        const rejectedSignalsList = [];
 ...
 */
function getAssetsForCycle(_topN, metadata) {
    let universe = [];
 ...
 */
function computePctChangeOverMinutesFromHistorical(historicalData, minutes) {
    try {
