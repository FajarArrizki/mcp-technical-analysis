...
function withLimitParam(basePath, limit) {
    if (!limit || limit <= 0)
        return basePath;
 ...
 */
async function httpsRequest(hostname, path, retries = 0) {
 ...
 */
export async function fetchFundingRate(asset) {
    ...
    try {
        ...
        const currentData = await httpsRequest('fapi.binance.com', `/fapi/v1/premiumIndex?symbol=${symbol}`, MAX_RETRIES);
        ...
        const rates = Array.isArray(historicalData) ? historicalData.map((r) => parseFloat(r.fundingRate || '0')) : [];
        // Calculate 24h and 7d averages
        const rate24h = rates.slice(0, 3).reduce((sum, r) => sum + r, 0) / Math.max(1, Math.min(3, rates.length));
        const rate7d = rates.slice(0, 8).reduce((sum, r) => sum + r, 0) / Math.max(1, Math.min(8, rates.length));
 ...
 */
export async function fetchOpenInterest(asset) {
    const symbol = BINANCE_FUTURES_SYMBOLS[asset];
    if (!symbol) {
        throw new Error(`Asset ${asset} not supported on Binance Futures`);
    }
    try {
        ...
        const currentData = await httpsRequest('fapi.binance.com', `/fapi/v1/openInterest?symbol=${symbol}`, MAX_RETRIES);
 ...
 */
export async function fetchLongShortRatio(asset) {
    const symbol = BINANCE_FUTURES_SYMBOLS[asset];
    if (!symbol) {
        throw new Error(`Asset ${asset} not supported on Binance Futures`);
    }
    try {
        ...
        const sentiment = longPct > 70 ? 'extreme_long' : longPct < 30 ? 'extreme_short' : 'balanced';
        return {
 ...
 */
export async function estimateLiquidationData(asset, currentPrice) {
 ...
 */
export async function fetchPremiumIndex(asset) {
    const symbol = BINANCE_FUTURES_SYMBOLS[asset];
    if (!symbol) {
        throw new Error(`Asset ${asset} not supported on Binance Futures`);
    }
    try {
        const premiumData = await httpsRequest('fapi.binance.com', `/fapi/v1/premiumIndex?symbol=${symbol}`, MAX_RETRIES);
        ...
        const historicalData = await httpsRequest('fapi.binance.com', `/fapi/v1/fundingRate?symbol=${symbol}&limit=30`, MAX_RETRIES);
        const premiums = Array.isArray(historicalData) ? historicalData.map((r) => parseFloat(r.fundingRate || '0')) : [];
        const premium24h = premiums.slice(0, 3).reduce((sum, p) => sum + p, 0) / Math.max(1, Math.min(3, premiums.length));
        const premium7d = premiums.slice(0, 8).reduce((sum, p) => sum + p, 0) / Math.max(1, Math.min(8, premiums.length));
        let trend = 'neutral';
        ...
        const stdDev = Math.sqrt(premiums.slice(0, 8).reduce((sum, p) => sum + Math.pow(p - avg7d, 2), 0) / Math.max(1, premiums.length));
 ...
 */
export async function fetchFuturesData(asset, currentPrice) {
 ...
 */
export async function batchFetchFuturesData(assets, prices) {
