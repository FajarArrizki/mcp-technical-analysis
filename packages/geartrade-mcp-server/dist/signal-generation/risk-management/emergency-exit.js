 ...
 */
export function checkLiquidationRisk(position, minBuffer = 5 // 5% minimum buffer
) {
    try {
        const liquidationCalc = calculateLiquidationPrice(position.entryPrice, position.leverage, position.side, 0 // No margin buffer for emergency check
        );
 ...
 */
export function checkFundingRateExtreme(futuresData, threshold = 0.002 // 0.2% threshold
) {
 ...
 */
export async function checkBTCShock(position, correlationData, _futuresData) {
    try {
 ...
 */
export async function checkEmergencyExitConditions(position, futuresData, correlationData) {
    const conditions = [];
    // Check liquidation risk
 ...
 */
export function shouldTriggerEmergencyExit(conditions) {
