 ...
 */
export function hasReversalConfirmations(indicators) {
    if (!indicators) {
        return false;
    }
    ...
    if (rsiDiv && rsiDiv.includes('bullish'))
        confirmations++;
    const macdDiv = indicators.macdDivergence?.divergence?.toLowerCase();
    if (macdDiv && macdDiv.includes('bullish'))
        confirmations++;
 ...
 */
export function getReversalConfirmationCount(indicators) {
    if (!indicators) {
        return 0;
    }
    let count = 0;
    ...
    if (macdDiv && macdDiv.includes('bullish'))
        count++;
    if ((indicators.volumeChange || 0) > 0.3)
        count++;
    if (indicators.rsi14 !== null && indicators.rsi14 !== undefined && indicators.rsi14 < 20) {
        count++;
    }
