 ...
 */
export function generateJustificationFromIndicators(signal, indicators, _bullishCount = 0, _bearishCount = 0, trendAlignment, externalData) {
    ...
    if (signal.signal === 'buy_to_enter' || signal.signal === 'add') {
        ...
        if (indicators?.candlestickPatterns &&
            ...
            const bearishPatterns = patterns.filter((p) => p.type && (p.type === 'bearish_engulfing' ||
                (p.type === 'doji' && p.bullish === false)));
            if (bearishPatterns.length > 0) {
                bearishIndicators.push(`Candlestick Patterns: ${bearishPatterns.map((p) => p.type).join(', ')} (bearish)`);
                actualBearishCount++;
            }
            if (bullishPatterns.length > 0) {
                // OPTIMIZATION 100x: Replace map().join() with for loop + join (faster)
