 ...
 */
export async function generateSignalForSingleAsset(asset, marketData, accountState, positions, equalCapitalPerSignal, signalIndex, allowedAssets, marketDataIsMap, // OPTIMIZATION: Pre-computed Map check to avoid repeated instanceof
assetRank = null, // PHASE 1: Asset rank for confidence calculation
qualityScore = null // PHASE 1: Quality score for confidence calculation
    ...
    try {
        ...
        if (indicators && signal && (signal.signal === 'buy_to_enter' || signal.signal === 'sell_to_enter')) {
            ...
            if (expectedDirection && signal.signal !== expectedDirection) {
                ...
                if (contradictionSeverity === 'SEVERE' && indicatorDifference >= 3) {
                    ...
                    const originalSignal = signal.signal;
                    signal.signal = expectedDirection;
        ...
        const processedSignal = await processSignal(signal, marketData, accountState, equalCapitalPerSignal, signalIndex, assetRank, // PHASE 1: Pass asset rank for confidence calculation
