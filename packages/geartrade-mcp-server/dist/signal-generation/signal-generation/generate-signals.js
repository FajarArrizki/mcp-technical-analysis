...
export async function generateSignals(_model, marketData, accountState, allowedAssets, _signalHistory = new Map(), positions, // OPTIMIZATION: Accept pre-computed positions to avoid duplicate call
rankedScores // PHASE 1: Pass ranked scores for confidence calculation
    ...
    const signalPromises = allowedAssets.map((asset, index) => {
        ...
        return generateSignalForSingleAsset(asset, marketData, accountState, positionsMap, // Use optimized positionsMap
        equalCapitalPerSignal, index, allowedAssets, marketDataIsMap, // OPTIMIZATION: Pass pre-computed Map check to avoid repeated instanceof
    ...
    if (signalsLenAtStart > 0) {
        ...
        const firstSignalAny = firstSignal;
    ...
    try {
        ...
        const correlationMap = hasCorrelation && correlationKeysLength > 1 ? new Map() : null;
        if (correlationMap && correlationKeysLength > 0) {
            ...
            for (let i = 0; i < correlationKeysLength; i++) {
                ...
                const corr = correlationMatrix[pair];
            ...
            for (let signalIdx = 0; signalIdx < signalDataLen; signalIdx++) {
                ...
                for (let i = 0; i < signalIdx; i++) {
                    ...
                    if (isContradictory) {
                        ...
                        const correlation = correlationMap.get(currentLess ? currentAsset + '-' + otherAsset : otherAsset + '-' + currentAsset);
        ...
        ;
        global.__rejectedSignals = rejectedSignals;
        if (totalRejected > 0) {
