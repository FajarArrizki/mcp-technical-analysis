...
export function calculateConfidenceScore(signal, indicators, trendAlignment, marketRegime, riskRewardRatio, externalData = null, assetRank = null, qualityScore = null) {
    let score = 0;
    let maxScore = 0;
    const breakdown = [];
    let trendScore = 0;
    ...
    if (trendAlignment) {
        ...
        const h4Aligned = trendAlignment.h4Aligned || false;
        const h1Aligned = trendAlignment.h1Aligned || false;
    ...
    if (externalData && indicators) {
        ...
        const orderBook = externalData.orderBook;
        ...
        const volumeDelta = externalData.volumeDelta;
        ...
        if (externalData.futures && indicators && indicators.price) {
            try {
                const futuresData = externalData.futures;
                const futuresMarketData = futuresData ? {
                    asset: signal.coin || '',
    ...
    if (extDataAny?.comprehensiveVolumeAnalysis || extDataAny?.enhancedMetrics?.volumeTrend) {
        ...
        if (isBuySignal) {
            ...
            if (volAnalysis?.liquidityZones && Array.isArray(volAnalysis.liquidityZones) && price > 0) {
                const nearbyResistance = volAnalysis.liquidityZones.find((zone) => {
