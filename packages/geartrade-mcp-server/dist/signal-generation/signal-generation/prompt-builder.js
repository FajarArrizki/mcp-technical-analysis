 ...
 */
export function buildTechnicalAnalysisSummary(asset, data, positions) {
    ...
    const analysisLines = [`**${asset} Technical Analysis:**`];
    if (!hasIndicators) {
    ...
    if (assetData && assetData.externalData) {
        ...
        if (ext.volumeProfile && ext.volumeProfile.session && price > 0) {
            ...
            const hvnParts = [];
            if (svp.hvn?.[0])
                hvnParts.push(`$${svp.hvn[0].price.toFixed(2)}`);
            if (svp.hvn?.[1])
                hvnParts.push(`$${svp.hvn[1].price.toFixed(2)}`);
            const hvnStr = hvnParts.length > 0 ? hvnParts.join(',') : 'N/A';
            const lvnParts = [];
            if (svp.lvn?.[0])
                lvnParts.push(`$${svp.lvn[0].price.toFixed(2)}`);
            if (svp.lvn?.[1])
                lvnParts.push(`$${svp.lvn[1].price.toFixed(2)}`);
 ...
 */
export function buildActivePositionsSummary(positions, marketData) {
...
export function buildSystemPromptForAsset(asset, marketData, _accountState, positions, allowedAssets) {
 ...
 */
export function buildUserPromptForAsset(asset, marketData, accountState, positions) {
    // OPTIMIZATION: Cache Map type check to avoid repeated instanceof
    const marketDataIsMap = marketData instanceof Map;
    ...
    // Add account state
    userPromptParts.push('', `**ACCOUNT**: Value $${accountValue.toFixed(2)} | Cash $${availableCash.toFixed(2)} | Return ${totalReturnPercent.toFixed(2)}% | Positions ${activePositionsCount} | Sharpe ${sharpeRatio.toFixed(2)}`, '', '**INSTRUCTIONS:**', '', 'Based on the TECHNICAL ANALYSIS data provided above, generate a trading signal for this asset.', '', '- Use RSI to identify overbought/oversold conditions', '- Use MACD to identify momentum and trend direction', '- Use EMA crossovers to identify trend changes', '- Use Bollinger Bands to identify volatility and potential reversals', '- Consider volume changes and price movements', '- Set appropriate stop losses and take profits based on technical levels', '', 'Generate a trading signal for this asset.');
