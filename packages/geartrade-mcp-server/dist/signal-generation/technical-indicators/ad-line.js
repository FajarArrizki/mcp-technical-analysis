...
export function calculateADLine(highs, lows, closes, volumes) {
    ...
    if (closes.length >= 10) {
        ...
        if (change > Math.abs(adLine) * 0.01)
            trend = 'accumulation'; // Significant upward movement
        else if (change < -Math.abs(adLine) * 0.01)
            trend = 'distribution'; // Significant downward movement
        else
            trend = 'neutral';
    }
    return {
