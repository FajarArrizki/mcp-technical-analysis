...
export function calculateCOG(closes, period = 10) {
    ...
    if (priceDiff > 0.02)
        signal = 'overbought'; // Price significantly above COG
    else if (priceDiff < -0.02)
        signal = 'oversold'; // Price significantly below COG
    else
        signal = 'neutral';
    // Determine trend based on COG position relative to price
    let trend = null;
    if (currentPrice > Math.abs(cog))
        trend = 'bullish';
    else if (currentPrice < Math.abs(cog))
        trend = 'bearish';
    else
        trend = 'neutral';
    return {
