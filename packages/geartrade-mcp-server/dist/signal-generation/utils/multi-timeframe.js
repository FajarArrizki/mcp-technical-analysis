 ...
 */
import { calculateTechnicalIndicators } from '../technical-indicators/aggregator';
export function calculateMultiTimeframeIndicators(multiTimeframeData, currentPrice) {
    const indicators = {};
...
export function checkTrendAlignment(multiTimeframeIndicators) {
    ...
    if (dailyTrend !== 'neutral') {
        ...
        if (h4Aligned)
            alignmentScore += 30; // 4H aligned (30 points)
        if (h1Aligned)
            alignmentScore += 30; // 1H aligned (30 points)
    }
    else {
