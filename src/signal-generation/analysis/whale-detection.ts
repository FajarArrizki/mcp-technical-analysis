...
export interface WhaleDetectionResult {
  largeOrders: WhaleOrder[]
  spoofingDetected: boolean
  washTradingDetected: boolean
  accumulationZones: Array<{ priceLow: number; priceHigh: number }>
  distributionZones: Array<{ priceLow: number; priceHigh: number }>
  smartMoneyFlow: number // -1 to 1, positive = accumulation
  whaleScore: number // 0-1, whale activity score
}
...
export function detectLargeOrders(
  ...
  minSize: number = LARGE_ORDER_THRESHOLD
): WhaleOrder[] {
  const largeOrders: WhaleOrder[] = []
  ...
  for (let i = 0; i < historicalData.length; i++) {
    ...
    if (volume > avgVolume + 2 * stdDev) {
      ...
      if (estimatedSize >= minSize) {
        ...
        const side: 'buy' | 'sell' = candle.close > candle.open ? 'buy' : 'sell'
 ...
 */
export function detectSpoofing(orders: WhaleOrder[]): boolean {
  ...
  const orderGroups = new Map<string, WhaleOrder[]>()
 ...
 */
export function identifyAccumulationDistributionZones(
  historicalData: HistoricalDataPoint[],
  largeOrders: WhaleOrder[]
): {
  accumulationZones: Array<{ priceLow: number; priceHigh: number }>
  ...
  const accumulationZones: Array<{ priceLow: number; priceHigh: number }> = []
  const distributionZones: Array<{ priceLow: number; priceHigh: number }> = []
 ...
 */
function clusterOrdersByPrice(
  orders: WhaleOrder[],
  rangePct: number
): Array<{ priceLow: number; priceHigh: number; size: number }> {
  const clusters: Array<{ priceLow: number; priceHigh: number; size: number; orders: WhaleOrder[] }> = []
...
export function analyzeWhaleActivity(
  ...
  // Identify accumulation/distribution zones
  const { accumulationZones, distributionZones } = identifyAccumulationDistributionZones(
    historicalData,
    largeOrders
  )

  // Calculate smart money flow
  const smartMoneyFlow = calculateSmartMoneyFlow(
    largeOrders,
    accumulationZones,
    distributionZones
  )
