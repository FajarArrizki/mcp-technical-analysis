...
export interface PositionMonitoringResult {
  ...
  fundingImpact24h: number // Funding cost/income for 24h (%)
  btcCorrelationImpact: number // Predicted impact from BTC move (%)
  riskLevel: 'low' | 'medium' | 'high' | 'critical'
  emergencyExit: {
    triggered: boolean
    reason: string
    action: 'close_all' | 'reduce_50' | 'reduce_25' | 'hold'
  }
  warnings: string[]
...
export async function monitorPosition(
  ...
  try {
    const liquidationCalc = calculateLiquidationPrice(
      ...
      position.leverage,
      position.side,
      0 // No margin buffer for monitoring
    )
  ...
  const emergencyConditions = await checkEmergencyExitConditions(
    position,
    futuresData,
    correlationData
  )
 ...
 */
export async function batchMonitorPositions(
  positions: Map<string, PositionState> | Record<string, PositionState>,
  futuresDataMap: Map<string, FuturesMarketData> | Record<string, FuturesMarketData>,
  correlationDataMap?: Map<string, BTCCorrelationData> | Record<string, BTCCorrelationData>
): Promise<Map<string, PositionMonitoringResult>> {
  const results = new Map<string, PositionMonitoringResult>()
