...
export interface IndicatorBasedConfig {
  enabled: boolean
  strict: boolean // Strict mode: exit on single indicator signal
  requireConfirmation: boolean // Require multiple indicators confirmation
  rsiThreshold: number // RSI exit threshold (default: 70 for LONG exit, 30 for SHORT exit)
  macdCrossover: boolean // Exit on MACD crossover in opposite direction
  emaBreak: boolean // Exit on EMA break
  supportResistanceBreak: boolean // Exit on support/resistance break
  atrExpansion: boolean // Exit on extreme ATR expansion
  atrExpansionThreshold: number // ATR expansion threshold (default: 2x average)
}

export function checkIndicatorBasedExit(
  position: PositionState,
  marketData: MarketData | null,
  config: IndicatorBasedConfig
): ExitCondition | null {
  if (!config.enabled) {
    return null
  }
  ...
  const exitConditions: Array<{ indicator: string; reason: string }> = []
  ...
  return {
    reason: 'INDICATOR_BASED' as ExitReason,
