/**
 * Run Live Mode
 * Entry point for live trading cycle
 */
...
async function main() {
  console.log('🚀 Starting LIVE MODE (Real Trading)')
  console.log('═══════════════════════════════════════\n')

  // Load configuration
  const cycleIntervalMs = parseInt(process.env.CYCLE_INTERVAL_MS || '300000') // 5 minutes default (keep longer for LIVE mode)
  const topNAssets = parseInt(process.env.TOP_ASSETS_FOR_AI || '15')
  const stateFile = process.env.CYCLE_STATE_FILE || 'data/cycle-state-live.json'
  const performanceFile = process.env.PERFORMANCE_REPORT_FILE || 'data/performance-report-live.json'
  const aiTrainingDataFile = process.env.AI_TRAINING_DATA_FILE || 'data/ai-training-data-live.json'
  const liveTradesFile = process.env.LIVE_TRADES_FILE || 'data/live-trades.json'
  ...
  if (!aiProviderApiKey) {
    console.error('❌ Error: AI_PROVIDER_API_KEY or OPENROUTER_API_KEY is required')
    console.error('   Set it with: export AI_PROVIDER_API_KEY=your_api_key')
    process.exit(1)
  }
  ...
  const initResult = await initializeLiveMode(config, model)

  if (initResult.error) {
    console.error(`❌ Failed to initialize live mode: ${initResult.error}`)
    process.exit(1)
  }

  const { state, executor, performanceTracker, cycleConfig } = initResult
  ...
  console.log(`\n⚠️  WARNING: This is LIVE trading mode with real funds!`)
  console.log(`═══════════════════════════════════════\n`)
  ...
  while (isRunning) {
    try {
      ...
      const result = await runLiveModeCycle(currentState, cycleConfig, performanceTracker)
      ...
      if (result.success) {
        ...
        // For live mode, get actual account value from executor if available
        let totalCapital = 10000 // Default fallback
        try {
          const accountInfo = await (executor as any).getAccountInfo?.()
          if (accountInfo && typeof accountInfo.accountValue === 'number') {
            totalCapital = accountInfo.accountValue
          }
        } catch {
          // Fallback to default if executor doesn't have getAccountInfo
        }
        ...
        if (filteredSignalsForDisplay.length > 0) {
          ...
          // Get actual account state from executor if available
          let accountState: any = {
            accountValue: totalCapital,
            availableCash: availableMargin,
            totalReturnPercent: performanceTracker.getMetrics().totalReturnPct || 0,
            activePositions: Array.from((currentState.positions || new Map()).values()),
            sharpeRatio: performanceTracker.getMetrics().sharpeRatio || 0
          }
          ...
          try {
            ...
            if (accountInfo) {
              accountState = {
                accountValue: accountInfo.accountValue || totalCapital,
                availableCash: accountInfo.availableCash || availableMargin,
                totalReturnPercent: accountInfo.totalReturnPercent || performanceTracker.getMetrics().totalReturnPct || 0,
                activePositions: accountInfo.activePositions || Array.from((currentState.positions || new Map()).values()),
                sharpeRatio: accountInfo.sharpeRatio || performanceTracker.getMetrics().sharpeRatio || 0
              }
            }
          } catch {
            // Use default accountState if executor doesn't have getAccountInfo
          }
          ...
          for (let i = 0; i < filteredSignalsForDisplay.length; i++) {
            ...
            await formatSignal(
              ...
              accountState, // accountState for LIVE mode (from executor)
              null // metadata
            )
          }
