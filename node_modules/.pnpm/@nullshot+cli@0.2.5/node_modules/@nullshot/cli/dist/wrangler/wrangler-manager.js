...
export class WranglerManager {
    ...
    async readConfig() {
        try {
            ...
            const defaultConfig = {
                name: "mcp-worker",
                compatibility_date: new Date().toISOString().split("T")[0] ?? "2025-05-15",
        ...
        try {
            ...
            throw new CLIError(`Failed to parse wrangler.jsonc: ${error instanceof Error ? error.message : String(error)}`, "Ensure the file contains valid JSONC syntax", 1);
        }
    }
    async writeConfig(config) {
        try {
            ...
            throw new CLIError(`Failed to save wrangler.jsonc: ${error instanceof Error ? error.message : String(error)}`, "Check file permissions and disk space", 1);
        }
    }
    /**
   * Update configuration by merging dependency wrangler configs and handling cleanup
   */
    async updateConfigWithDependencies(dependencyConfigs) {
        ...
        const serviceNameToServerName = new Map();
        ...
        for (const depConfig of dependencyConfigs) {
            if (depConfig.name) {
                ...
                if (oldMetadata && oldMetadata.serviceName && oldMetadata.serviceName !== depConfig.name) {
                    ...
                    const oldServiceIndex = wranglerConfig.services.findIndex(service => service.binding === oldServiceBindingName);
                ...
                const existingService = wranglerConfig.services.find(service => service.binding === serviceBindingName);
                if (!existingService) {
            ...
            if (depConfig.compatibility_flags && wranglerConfig.compatibility_flags) {
                ...
                const newFlags = depConfig.compatibility_flags.filter(flag => !currentFlags.includes(flag));
     ...
     */
    ensureWranglerStructure(wranglerConfig) {
        if (!wranglerConfig.services) {
...
//# sourceMappingURL=wrangler-manager.js.map