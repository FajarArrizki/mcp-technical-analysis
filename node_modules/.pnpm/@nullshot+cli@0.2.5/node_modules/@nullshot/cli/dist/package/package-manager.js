...
export class PackageManager {
    ...
    async installPackage(source, serverName) {
        ...
        const isAlreadyInstalled = allDependencies[extractedName] === source ||
        ...
        try {
            ...
            throw new CLIError(`Failed to install package ${source}: ${error instanceof Error ? error.message : String(error)}`, `Check that the source URL is valid and accessible`, 1);
        }
    }
    async removePackage(packageName) {
        ...
        try {
            ...
            throw new CLIError(`Failed to remove package ${packageName}: ${error instanceof Error ? error.message : String(error)}`, "Check that the package is actually installed", 1);
        }
    }
    async getInstalledMCPPackages() {
        try {
            const packageJson = await this.readPackageJson();
    ...
    async getMCPPackageMetadata(serverName) {
        try {
            const packageJson = await this.readPackageJson();
    ...
    async cleanupRemovedServers(currentServers) {
        const installedServers = await this.getInstalledMCPPackages();
        const serversToRemove = installedServers.filter((server) => !currentServers.includes(server));
        for (const serverName of serversToRemove) {
            try {
                ...
                logger.warn(`Failed to remove server ${serverName}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    async readPackageJson() {
        try {
            ...
            throw new CLIError("No package.json found in current directory", "Make sure you are in a Node.js project directory with a package.json file", 1);
        }
    }
    async writePackageJson(packageJson) {
        try {
            ...
            throw new CLIError(`Failed to update package.json: ${error instanceof Error ? error.message : String(error)}`, "Check file permissions and disk space", 1);
        }
    }
    async addMCPMetadata(serverName, source, analysis) {
        const packageJson = await this.readPackageJson();
    ...
    async removeMCPMetadata(serverName) {
        const packageJson = await this.readPackageJson();
        if (packageJson.mcpServers) {
     ...
     */
    async analyzeDependency(source, serverName) {
        try {
     ...
     */
    async getDependenciesWithWranglerConfigs() {
     ...
     */
    async addToPostinstall(command) {
        const packageJson = await this.readPackageJson();
        if (!packageJson.scripts) {
            packageJson.scripts = {};
        }
