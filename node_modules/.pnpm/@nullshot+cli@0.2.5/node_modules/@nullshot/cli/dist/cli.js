...
program
    ...
    .option("--skip-wrangler-update", "Skip updating wrangler.jsonc configuration")
    ...
    const { dryRun, verbose, config: configPath, cwd, } = program.opts();
...
program
    ...
    .action(async (options) => {
    const { config: configPath, cwd } = program.opts();
    // Change to the specified working directory
    const originalCwd = process.cwd();
    if (cwd && cwd !== originalCwd) {
        process.chdir(cwd);
    ...
    try {
        const configManager = new ConfigManager(configPath || "mcp.json");
        const servers = await listInstalledServers(configManager, options.format ?? "table");
        console.log(servers);
    }
    catch (error) {
        handleError(error);
...
createCommand
    ...
    try {
        const { dryRun, verbose } = program.opts();
        const dryRunManager = new DryRunManager(dryRun || false);
        if (verbose)
            logger.setVerbose(true);
        if (dryRun)
...
//     }
//   });
async function installServers(config, context) {
    const { dryRunManager, packageManager, wranglerManager, skipPackageUpdate, skipWranglerUpdate, spinner, } = context;
    ...
    if (!skipPackageUpdate) {
        ...
        for (const [name, serverConfig] of Object.entries(config.mcpServers)) {
            ...
            if (serverConfig.source) {
                await dryRunManager.execute(`Install package ${serverConfig.source}`, () => packageManager.installPackage(serverConfig.source, name));
            }
        }
    }
    ...
    if (!skipWranglerUpdate) {
        ...
        await dryRunManager.execute("Update wrangler.jsonc with MCP server bindings", async () => {
    ...
    await dryRunManager.execute("Remove packages not in configuration", async () => {
    ...
    const hasCfTypegen = await packageManager.hasScript("cf-typegen");
    if (hasCfTypegen) {
        await dryRunManager.execute("Generate Wrangler types using cf-typegen", async () => {
        ...
        logger.warn(chalk.yellow("⚠️  Skipping generating wrangler types since cf-typegen does not exist. " +
            'You must generate manually or add "cf-typegen": "wrangler types" to your package.json'));
    }
}
async function listInstalledServers(configManager, format) {
    const config = await configManager.load();
    const packageManager = new PackageManager();
    ...
    // Table format
    const table = servers
        .map((server) => `${server.status === "installed" ? "✅" : "! "} ${server.name.padEnd(20)} ${(server.source || server.url || "").padEnd(40)} ${server.command || ""}`)
        .join("\n");
    return `${"Name".padEnd(22)} ${"Source".padEnd(42)} Command\n${"─".repeat(80)}\n${table}`;
}
async function runDev(config, options) {
    const { dryRunManager, spinner } = options;
    if (!config.mcpServers || Object.keys(config.mcpServers).length === 0) {
        throw new CLIError("No MCP servers found in configuration", "Run 'nullshot install' to add some MCP servers or check your mcp.json file", 1);
    }
    ...
    const failedD1Migrations = d1MigrationResults.filter((result) => !result.success);
    ...
    return new Promise((resolve, reject) => {
        childProcess.on("close", (code) => {
            if (code === 0) {
                ...
                reject(new CLIError(`Wrangler dev exited with code ${code}`, "Check the logs above for error details", code || 1));
            }
        });
        childProcess.on("error", (error) => {
            reject(new CLIError(`Failed to start wrangler dev: ${error.message}`, "Make sure wrangler is installed and accessible in your PATH", 1));
        });
...
function handleError(error) {
    if (error instanceof CLIError) {
        ...
        logger.error(chalk.red(`❌ Unexpected error: ${error instanceof Error ? error.message : String(error)}`));
        process.exit(1);
    }
}
